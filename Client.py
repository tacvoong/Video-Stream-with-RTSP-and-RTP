...
from tkinter import messagebox, Button, Label, N, E, W, S
...
class Client:
    ...
    def updateMovie(self, data):
        ...
        photoData = Image.open(BytesIO(data))
        photo = ImageTk.PhotoImage(photoData)
            ...
            if self.sessionId == session:
                if int(lines[0].split(" ")[1]) == 200:
                    if self.requestSent == self.SETUP:
                        ...
                        sdpFileStr = "\n".join(lines[7:])
                        messagebox.showinfo(
                            title="Stream information", message=sdpFileStr
                        )
    ...
    def handler(self):
        ...
        self.pauseMovie()
        while self.state != self.READY and self.state != self.INIT:
            pass
